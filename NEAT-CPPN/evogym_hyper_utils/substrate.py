...
ObservationDimensions = {
    ...
    'floor'                     : ['vertical', 'y', 'pos_x'],
    'ceil'                      : ['vertical', 'y', 'pos_x'],
...
class Substrate():
    ...
    def make_substrate(self):
            ...
            if obs=='robot_velocity':
                ...
                for x in range(-self.sight_depth, self.sight_depth+1):
                    self.nodes['input'].append(
                        {
                            'name': f'floor_{x: =+}',
                            'dims': {'sight': 1, 'y': 1, 'pos_x': x, 'pos_y': -1},
                            'vector': None
                        })
        ...
        for i,actuator in enumerate(self.actuators):
            self.nodes['output'].append(
                {
                    'name': f'actuator{i+1}',
                    'dims': {'pos_x': actuator[0], 'pos_y': actuator[1], 'horizontal': actuator[2], 'vertical': actuator[3]},
                    'vector': None
                })
